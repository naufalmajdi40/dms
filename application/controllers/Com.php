...
class Com extends CI_Controller
{
	...
	function __construct()
    {
		...
		$this->load->model('Config_model'); //memanggil config model  cuyy
	...
	public function save_com(){
		...
		$res=$this->Config_model->save_config($data);
		if($res){
			echo ('Update Success');
		}
		else{
   ...
   public function index(){
	...
	$row['data']=$this->Config_model->get_all_selected();

	$this->load->view('header',$dataAdm);	 // Menampilkan bagian header dan object data users
	$this->load->view('data/com_type',$row); // Menampilkan config com
